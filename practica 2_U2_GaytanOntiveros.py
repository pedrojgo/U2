...
df = pd.read_csv('C:/Users/HP/Downloads/mi_archivo.csv') 

# Visualizar los datos cargados para verificar
print(df)

# Seleccionar las características y las etiquetas para el entrenamiento
# Aquí se hace una transformación simple para cambiar las frutas a etiquetas numéricas
frutas = {'Manzana': 1, 'Pera': 2, 'Banana': 3, 'Naranja': 4, 'Platano': 5}
df['Tipo_de_Fruta'] = df['Tipo_de_Fruta'].map(frutas)

y = df['Tipo_de_Fruta'].values
X = df[['Precio', 'Peso']].values
    ...
    def fit(self, X, y):
        ...
        for _ in range(self.n_iter):
            ...
            for xi, target in zip(X, y):
                update = self.eta * (target - self.predict(xi.reshape(1, -1))[0])
                self.w_[1:] += update * xi
                self.w_[0] += update
